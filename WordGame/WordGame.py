...
def load_game_words() -> None:
    ...
    try:
        with open("words.txt", 'r') as words_file:
            [all_words.append(line.strip()) for line in words_file.readlines()]
...
def handle_score(gameword: str) -> int:
    ...
    if (gameword not in all_words) or (len(gameword) < 4) \
        or (gameword in correct_guesses):
            return 0;
