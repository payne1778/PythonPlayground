...
def is_monster_at_coords(x: int, y: int, monsters: list[Actor]) -> bool:
	if len(monsters) == 0: return False

	for monster in monsters:
		if monster.x == x and monster.y == y: return True
	return False
...
def make_monsters(size: int) -> list[Actor]:
	...
	for i in range(0, num_monsters):
		...
		new_monster: Actor = (
			Actor(name=f"Monster {i}", health=25, max_damage=5, x=x, y=y)
		)
...
def count_nearby_monsters(hero: Actor, monsters: list[Actor]) -> int:
	count: int = 0
	for monster in monsters:
		if hero.in_adjacent_room(monster): count += 1
	return count; 
